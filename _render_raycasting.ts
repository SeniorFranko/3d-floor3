...
namespace Render {
    ...
    export class RayCastingRender {
        ...
        private setVectors() {
            ...
            this.dirX14 = Fx14(cos)
            this.dirY14 = Fx14(sin)
            this.planeX14 = Fx14(sin * this._fov)
            this.planeY14 = Fx14(cos * -this._fov)
          ...
          this.tempScreen.fill(0)
            ...
            for (let x = 0; x < SW; x++) {
               ...
               const tex = this.textures[color]
                ...
                drawStart = SHHalf + Math.ceil(drawStart)
                drawEnd += SHHalf
               ...
               // this.wallEnd[x] = Math.ceil(drawStart) + drawHeight
            ...
            for (let x = 0; x < SW; x++) {
                ...
                drawStart = F14.toFloat(F14.sub(drawEnd14,lineHeight14)) ;
                drawHeight = Math.ceil (Math.floor(F14.toFloat(drawEnd14)) - Math.ceil(drawStart))
                drawStart = SHHalf + Math.ceil(drawStart)
                //drawEnd += SHHalf
...
*/
           ...
           // let test = this.spriteMotionZ[this.sprSelf.id].p//(2 << fpx) + this.cameraOffsetZ_fpx//(this.sprSelf._height as any as number)
            this.tempScreen.print(this.f14precision.toString(), 0,0,7 )
        ...
        drawSprite(spr: Sprite, index: number, transformX: number, transformY: number, myAngle: number) {
            ...
            const drawStart = SHHalf + (lineHeight * ((F14.toFloat(this.viewZPos14) - this.spriteMotionZ[spr.id].p - (spr._height as any as number)) / this.tilemapScaleSize) >> fpx)
